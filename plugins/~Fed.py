import uuid
import asyncio
from Bad.database.feds_db import *
import os
from Bad import app
...
from Bad.database.fedfundb import extract_user, extract_user_and_reason
...
__MODULE__ = "ғᴇᴅᴇʀᴀᴛɪᴏɴ"
__HELP__ = """
**<u>👑 ғᴇᴅ ᴏᴡɴᴇʀ ᴄᴏᴍᴍᴀɴᴅꜱ</u>**

» `/newfed <name>` - ᴄʀᴇᴀᴛᴇ ᴀ ɴᴇᴡ ғᴇᴅ.
» `/renamefed <fed_id> <new_name>` - ʀᴇɴᴀᴍᴇ ᴀ ɢɪᴠᴇɴ ғᴇᴅ.
» `/delfed <fed_id>` - ᴅᴇʟᴇᴛᴇ ᴀ ғᴇᴅ ᴀɴᴅ ɪᴛꜱ ᴅᴀᴛᴀ.
» `/myfeds` - ʟɪꜱᴛ ᴀʟʟ ғᴇᴅꜱ ʏᴏᴜ ᴏᴡɴ.
» `/fedtransfer <new_owner> <fed_id>` - ᴛʀᴀɴꜱꜰᴇʀ ғᴇᴅ ᴏᴡɴᴇʀꜱʜɪᴘ.
» `/fpromote <user>` - ᴘʀᴏᴍᴏᴛᴇ ᴜꜱᴇʀ ᴛᴏ ғᴇᴅ ᴀᴅᴍɪɴ.
» `/fdemote <user>` - ʀᴇᴍᴏᴠᴇ ғʀᴏᴍ ғᴇᴅ ᴀᴅᴍɪɴ.
» `/setfedlog <fed_id>` - ꜱᴇᴛ ᴛʜɪꜱ ɢʀᴏᴜᴘ ᴀꜱ ғᴇᴅ ʟᴏɢ.
» `/unsetfedlog <fed_id>` - ʀᴇᴍᴏᴠᴇ ɢʀᴏᴜᴘ ꜰʀᴏᴍ ғᴇᴅ ʟᴏɢ.
» `/fbroadcast <msg>` - ʙʀᴏᴀᴅᴄᴀꜱᴛ ᴛᴏ ᴀʟʟ ғᴇᴅ ᴄʜᴀᴛꜱ.

**<u>🔱 ғᴇᴅ ᴀᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅꜱ</u>**

» `/fban <user> <reason>` - ғᴇᴅʙᴀɴ ᴀ ᴜꜱᴇʀ.
» `/sfban` - ꜱɪʟᴇɴᴛ ғᴇᴅʙᴀɴ (ɴᴏ ɴᴏᴛɪᴄᴇ).
» `/unfban <user>` - ʀᴇᴍᴏᴠᴇ ᴜꜱᴇʀ ꜰʀᴏᴍ ғᴇᴅʙᴀɴ.
» `/sunfban` - ꜱɪʟᴇɴᴛ ᴜɴғʙᴀɴ (ɴᴏ ɴᴏᴛɪᴄᴇ).
» `/fedadmins` - ꜱʜᴏᴡ ғᴇᴅ ᴀᴅᴍɪɴꜱ.
» `/fedchats <fed_id>` - ꜱʜᴏᴡ ᴀʟʟ ᴄʜᴀᴛꜱ ɪɴ ᴛʜᴇ ғᴇᴅ.
» `/fbroadcast <msg>` - ʙʀᴏᴀᴅᴄᴀꜱᴛ ᴍꜱɢ ᴛᴏ ғᴇᴅ ᴄʜᴀᴛꜱ.

**<u>👥 ᴜꜱᴇʀ ᴄᴏᴍᴍᴀɴᴅꜱ</u>**

» `/fedinfo <fed_id>` - ɢᴇᴛ ɪɴꜰᴏ ᴀʙᴏᴜᴛ ᴀ ғᴇᴅ.
» `/fedadmins <fed_id>` - ꜱʜᴏᴡ ᴀᴅᴍɪɴꜱ ᴏꜰ ᴀ ғᴇᴅ.
» `/joinfed <fed_id>` - ᴊᴏɪɴ ᴛʜᴇ ғᴇᴅ (ᴄʜᴀᴛ ᴏᴡɴᴇʀ ᴏɴʟʏ).
» `/leavefed` - ʟᴇᴀᴠᴇ ᴛʜᴇ ғᴇᴅ (ᴄʜᴀᴛ ᴏᴡɴᴇʀ ᴏɴʟʏ).
» `/fedstat` - ᴄʜᴇᴄᴋ ʏᴏᴜʀ ғᴇᴅ ʙᴀɴ ꜱᴛᴀᴛᴜꜱ.
» `/fedstat <user> <fed_id>` - ᴄʜᴇᴄᴋ ᴜꜱᴇʀ's ʙᴀɴ ꜱᴛᴀᴛᴜꜱ.
» `/chatfed` - ꜱʜᴏᴡ ᴛʜᴇ ғᴇᴅ ᴏꜰ ᴄᴜʀʀᴇɴᴛ ᴄʜᴀᴛ.
"""
